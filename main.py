...
# 2. 에러 원천 차단형 뉴스 수집 (entry.summary 문구 자체를 아예 안 씀)
def get_recent_news():
    ...
    if not recent_articles:
        recent_articles.append("제목: 글로벌 금융 시장 동향\n요약: 주요 경제 지표 발표 대기 중\n출처: Yahoo Finance\n")

    return "\n---\n".join(recent_articles[:10])
...
def send_telegram(image_path):
    ...
    with open(image_path, "rb") as photo:
        res = requests.post(url, data={"chat_id": TELEGRAM_CHAT_ID}, files={'photo': photo})
