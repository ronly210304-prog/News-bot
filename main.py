...
# 2. 당일 & 최근 2시간 이내 실시간 뉴스만 엄선해서 수집 (속성 에러 원천 차단)
def get_recent_news():
    ...
    # 딱 오늘 하루 혹은 최근 3시간 이내에 올라온 진짜 실시간 뉴스만 필터링
    ...
    for entry in feed.entries:
        ...
        if published_parsed:
            ...
            # 당일(최근 3시간 이내) 뉴스인 경우에만 수집
            if pub_date >= time_limit:
                # 안전하게 데이터 추출 (summary가 없어도 절대 에러 안 나게 방어)
                title = entry.get('title', '제목 없음')
                summary = entry.get('summary', entry.get('description', '요약 내용 없음'))
                source_name = "Yahoo Finance"
                if hasattr(entry, 'source') and isinstance(entry.source, dict):
                    source_name = entry.source.get('title', 'Yahoo Finance')
                
                recent_articles.append(f"제목: {title}\n요약: {summary}\n출처: {source_name}\n")
    
    # 만약 최근 3시간 이내에 올라온 긴급 뉴스가 없다면, 가장 최신 뉴스 1~2개만 가져옴
