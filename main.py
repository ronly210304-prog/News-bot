...
# 2. 최근 2시간 동안의 뉴스 수집 (에러 방지 안전 장치 추가)
def get_recent_news():
    rss_url = "https://finance.yahoo.com/news/rssindex"
    feed = feedparser.parse(rss_url)
    now = datetime.now(timezone.utc)
    two_hours_ago = now - timedelta(hours=2, minutes=30) # 버퍼 넉넉히
    ...
    # 만약 2시간 이내에 뉴스가 없으면 최근 뉴스 몇 개라도 가져오기 (테스트 및 공백 방지용)
